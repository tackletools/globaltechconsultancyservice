...
$marketing_services = $_POST['marketing_services'] ?? [];
$marketing_prices_input = $_POST['marketing_prices'] ?? [];

// Reference data for type and name (keep this array to determine monthly or one-time)
$marketing_service_meta = [
    'graphic_design' => ['name' => 'Graphic Design (30 posts/month)', 'type' => 'monthly'],
    'social_media' => ['name' => 'Social Media Management', 'type' => 'monthly'],
    'seo' => ['name' => 'SEO Implementation', 'type' => 'one-time'],
    'ads_management' => ['name' => 'Ads Management (Google & Meta)', 'type' => 'monthly']
];

$marketing_one_time = 0;
$marketing_monthly = 0;
$selected_marketing_details = []; // for optional display

foreach ($marketing_services as $service) {
    $price = isset($marketing_prices_input[$service]) ? (float)$marketing_prices_input[$service] : 0;
    $type = $marketing_service_meta[$service]['type'];
    $name = $marketing_service_meta[$service]['name'];

    if ($type === 'one-time') {
        $marketing_one_time += $price;
    } else {
        $marketing_monthly += $price;
    }

    $selected_marketing_details[] = [
        'name' => $name,
        'price' => $price,
        'type' => $type
    ];
}
$total_one_time = $project_cost + $marketing_one_time;
        ...
        <div class="section header-section">
            <div class="row align-items-center">
                <div class="col-md-3 text-center">
                    <img src="assets/getcslogonew.png" alt="Global Tech Logo" class="company-logo">
                </div>
                <div class="col-md-9">
...
<?php if (!empty($selected_marketing_details)): ?>
<div class="section">
    <h2 class="section-title">
        <span class="icon"><i class="fas fa-bullhorn"></i></span>
        Digital Marketing Services (Optional)
    </h2>
    <p class="mb-3">Additional marketing services included in this quotation:</p>

    <?php foreach ($selected_marketing_details as $service): ?>
    <div class="marketing-service-item">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="service-price">
                ...
                <?php echo $service['type'] === 'monthly' ? '/month' : ' (one-time)'; ?>
            </div>
        </div>
    </div>
    <?php endforeach; ?>
...
<div class="section">
    ...
    <div class="cost-summary">
        ...
        <?php foreach ($selected_marketing_details as $service): ?>
            <?php if ($service['type'] === 'one-time'): ?>
            <div class="cost-row">
                <div class="cost-label">
                    <i class="fas fa-bullhorn me-2"></i>
                    <?php echo htmlspecialchars($service['name']); ?> (One-time)
                </div>
                <div class="cost-value">₹<?php echo number_format($service['price']); ?></div>
            </div>
            <?php endif; ?>
        <?php endforeach; ?>
